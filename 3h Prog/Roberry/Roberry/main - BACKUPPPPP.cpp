...
static int nodes_number, nodes[50000], nodes_hash[50000], nodes_fathers_and_costs[50000][3], visited_nodes;
...
void dfs_base(int cost, int current_node, int father, int real_conn[])
{
    int temporary_cost, temporary_hyper, temporary_father;
    nodes_hash[current_node] = 2;
    nodes_fathers_and_costs[current_node][0] = father;
    nodes_fathers_and_costs[current_node][1] = cost;
    nodes_fathers_and_costs[current_node][2] = nodes_fathers_and_costs[father][2] + 1;
    visited_nodes++;
    ...
    while(temp != NULL)
    {
        ...
        if(temp->node != father)
        {
            if(nodes_hash[temp->node] == 1)
            {
                ...
                dfs_base(temp->cost, temp->node, current_node, temp->real_connection);
            }else if(nodes_hash[temp->node] == 2 )
            {
                temp_temp = current_node;
                result[1] += temp->cost;
                //printf("We are here: %d. We add %d to the result2, because node: %d is already visited\n", temp_temp, temp->cost, temp->node);
                while(tree_find(temp_temp) != tree_find(temp->node))
                {
                    //Afairese to kostos apo to result[0] dhladh to an autos o kombos krinetai h oxi
                    result[0] -= nodes_fathers_and_costs[temp_temp][1];
                    result[1] += nodes_fathers_and_costs[temp_temp][1];
                    nodes_fathers_and_costs[temp_temp][1] = 0;
                    //Union
                    temporary_cost = nodes_fathers_and_costs[tree_find(nodes_fathers_and_costs[temp_temp][0])][1];
                    temporary_father = nodes_fathers_and_costs[tree_find(nodes_fathers_and_costs[temp_temp][0])][0];
                    temporary_hyper = tree_union(tree_find(temp_temp), tree_find(nodes_fathers_and_costs[temp_temp][0]));
                    //Put as cost and father of the union the father of the father
                    nodes_fathers_and_costs[temporary_hyper][1] = temporary_cost;
                    nodes_fathers_and_costs[temporary_hyper][0] = temporary_father;

                    temp_temp = temporary_hyper;
                }
            ...
            else if(nodes_hash[temp->node] == 4)
            {
...
void execute_dfs()
{
    ...
    while(visited_nodes < nodes_number)
    {
        for(i=j;i<nodes_number;i++)
        {
            ...
            {
                nodes_fathers_and_costs[nodes[i]][2] = 0;
                dfs_base(0, nodes[i], -1, real_conn);
                j = i;
                break;
            }
        }
...
void make_subgraph(int start, int end)
{
    ...
    {
        nodes_hash[i] = 0;
        nodes_fathers_and_costs[i][0] = -1;
        nodes_fathers_and_costs[i][1] = 0;
    }
